...
- `:default`が指定されている場合、オプション省略時に変数にはデフォルト値が束縛され、指定されていなければnilが束縛される。デフォルト値が整数の場合は文字列ではなく、readされたオブジェクトが変数に束縛される
...
こういった定型的なパターンを記述する方法がマクロライブラリとして実装できるところもCommon Lispをスクリプティングに使う際のメリットの一つであると考えられる。
